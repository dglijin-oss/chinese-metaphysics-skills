---
name: bazi-pan-skill
description: 八字命理排盘技能。支持四柱八字排盘、十神计算、大运排法、五行统计、格局判断、用神选取、流年推算、详细断语、神煞系统（33+神煞）、刑冲合害分析、流月推算、综合评分、趋吉避凶建议。
version: 4.0.0
author: 天工长老
---

# 八字排盘技能 v4.0.0

> "不知命，无以为君子也。" ——《论语》

## ⚠️ 铁律：先核对时间，再排盘！

> "差之毫厘，谬以千里。" —— 时间一错，全盘皆废。

**每次排盘前必须：**

1. **校正当前时间** — 以系统实时时间为准，不可用缓存或旧时间
2. **核实公历日期** — 年、月、日、时，一一确认
3. **核对农历转换** — 月份、日期不可想当然，需验证
4. **验证节气月令** — 月柱以节气为准（如清明后为辰月）
5. **独立校验脚本输出** — 脚本若有 bug，人工复核

---

## 1. Description

八字命理，又称四柱命理，是以出生年月日时的天干地支推算命运的术数。

**核心功能 v4.0.0：**
- ✅ **刑冲合害分析模块**（新增！）
  - 地支六合、六冲、三合、半三合、三刑、六害、相破
  - 流年与四柱的刑冲合害关系
  - 综合判断与建议
- ✅ **流月推算模块**（新增！）
  - 12个月流月干支计算
  - 每月运势评分（0-100）
  - 事业/财运/感情/健康分类建议
  - 最佳/最差月份标记
- ✅ **综合评分模块**（新增！）
  - 六维度评分：五行平衡、格局优劣、用神有力、神煞吉凶、刑冲合害、日主强弱
  - 总分0-100，七级评定（上上/上吉/中上/中平/中下/下平/下下）
- ✅ **趋吉避凶建议模块**（新增！）
  - 吉利方位、颜色、行业、数字
  - 吉祥物、吉利食物
  - 五行补救建议、注意事项
- ✅ **神煞系统**（33+神煞，识别率100%）
  - 贵人吉神：天乙贵人、太极贵人、天德贵人、月德贵人、三奇贵人、天赦
  - 文星学业：文昌、学堂、词馆
  - 禄财：禄神、金舆
  - 婚姻感情：桃花、红艳、红鸾、天喜
  - 动迁出行：驿马
  - 特殊格局：华盖、魁罡、金神、羊刃、飞刃、将星、国印贵人、六秀
  - 凶煞：空亡、孤辰、寡宿、十恶大败、披头、吊客、病符、丧门、八专
- ✅ 统一断语风格（古风简洁，切中要害）
- ✅ 技能联动接口（可调用紫微/奇门/六爻综合推断）
- ✅ 四柱排盘（年柱/月柱/日柱/时柱）
- ✅ 十神计算（比肩/劫财/食神/伤官等）
- ✅ 大运排法（8 步大运）
- ✅ 五行统计（木火土金水数量）
- ✅ 格局判断（正官格/七杀格/食神格等 10 种格局）
- ✅ 用神选取（日主旺衰、喜用神、忌神）
- ✅ 流年推算（当年运势、冲克分析）
- ✅ 详细断语（事业/婚姻/财运/健康）
- ✅ 五行缺失检测

**核心功能 v2.0.0：**
- ✅ 四柱排盘（年柱/月柱/日柱/时柱）
- ✅ 十神计算（比肩/劫财/食神/伤官等）
- ✅ 大运排法（8 步大运）
- ✅ 五行统计（木火土金水数量）
- ✅ 格局判断（正官格/七杀格/食神格等 10 种格局）
- ✅ 用神选取（日主旺衰、喜用神、忌神）
- ✅ 流年推算（当年运势、冲克分析）
- ✅ 详细断语（事业/婚姻/财运/健康）
- ✅ 五行缺失检测

**v1.0.0 基础功能：**
- ✅ 四柱排盘、十神、大运、五行统计、基础断语

---

## 2. When to use

| 场景 | 示例 |
|------|------|
| 排八字请求 | "帮我排一下八字" |
| 指定时间排盘 | "1990 年 5 月 15 日早上 8 点的八字" |
| 问命理 | "看看我的命格怎么样" |
| 五行查询 | "我五行缺什么" |
| 大运查询 | "我的大运怎么样" |

---

## 3. How to use

### 调用方式

```bash
# 基础排盘（含刑冲合害、综合评分、趋吉避凶）
python3 ~/.openclaw/skills/bazi-pan-skill/scripts/bazi_pan.py --date "1990-05-15" --hour 8

# 指定性别（影响大运顺逆）
python3 ~/.openclaw/skills/bazi-pan-skill/scripts/bazi_pan.py --date "1990-05-15" --hour 8 --gender 女

# 带流年分析
python3 ~/.openclaw/skills/bazi-pan-skill/scripts/bazi_pan.py --date "1990-05-15" --hour 8 --liu-nian 2026

# 带流月运势（显示12个月运势，需配合--liu-nian）
python3 ~/.openclaw/skills/bazi-pan-skill/scripts/bazi_pan.py --date "1990-05-15" --hour 8 --liu-nian 2026 --liu-yue

# 输出 JSON 格式
python3 ~/.openclaw/skills/bazi-pan-skill/scripts/bazi_pan.py --date "1990-05-15" --hour 8 --json
```

### 命令行参数

| 参数 | 简写 | 说明 |
|------|------|------|
| --date | -d | 出生日期 (YYYY-MM-DD) |
| --hour | -H | 出生时辰 (0-23) |
| --gender | -g | 性别 (男/女，默认男) |
| --liu-nian | -l | 流年年份（可选，如 2026） |
| --liu-yue | -m | 显示流月运势（需配合--liu-nian） |
| --json | -j | 输出 JSON 格式 |

### 时辰对照表

| 时辰 | 时间 | 地支 |
|------|------|------|
| 子时 | 23:00-01:00 | 子 |
| 丑时 | 01:00-03:00 | 丑 |
| 寅时 | 03:00-05:00 | 寅 |
| 卯时 | 05:00-07:00 | 卯 |
| 辰时 | 07:00-09:00 | 辰 |
| 巳时 | 09:00-11:00 | 巳 |
| 午时 | 11:00-13:00 | 午 |
| 未时 | 13:00-15:00 | 未 |
| 申时 | 15:00-17:00 | 申 |
| 酉时 | 17:00-19:00 | 酉 |
| 戌时 | 19:00-21:00 | 戌 |
| 亥时 | 21:00-23:00 | 亥 |

---

## 4. Output Format

```
【八字排盘】v1.0.0
• 出生时间：1990-05-15 08:00
• 性别：男

【四柱】
    年柱    月柱    日柱    时柱
    庚午    辛巳    壬辰    甲辰

【十神】
    偏印    正印    日主    食神

【五行统计】
    木：2  火：2  土：2  金：2  水：0
    ⚠️  五行缺：水

【大运】
    13 岁：壬午
    23 岁：癸未
    33 岁：甲申
    43 岁：乙酉
    53 岁：丙戌
    63 岁：丁亥
    73 岁：戊子
    83 岁：己丑

【基础断语】
• 【日主】壬水江河，聪明灵活，适应力强
• 【五行】金最旺（2 个），水最弱（0 个）
• 【注意】五行缺水，需后天补救
```

---

## 5. 八字基础知识

### 十天干

| 天干 | 甲 | 乙 | 丙 | 丁 | 戊 | 己 | 庚 | 辛 | 壬 | 癸 |
|------|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| 五行 | 木 | 木 | 火 | 火 | 土 | 土 | 金 | 金 | 水 | 水 |
| 阴阳 | 阳 | 阴 | 阳 | 阴 | 阳 | 阴 | 阳 | 阴 | 阳 | 阴 |

### 十二地支

| 地支 | 子 | 丑 | 寅 | 卯 | 辰 | 巳 | 午 | 未 | 申 | 酉 | 戌 | 亥 |
|------|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| 生肖 | 鼠 | 牛 | 虎 | 兔 | 龙 | 蛇 | 马 | 羊 | 猴 | 鸡 | 狗 | 猪 |
| 五行 | 水 | 土 | 木 | 木 | 土 | 火 | 火 | 土 | 金 | 金 | 土 | 水 |

### 十神关系

| 十神 | 关系 | 含义 |
|------|------|------|
| 比肩 | 同我者，阴阳同 | 兄弟、朋友、竞争 |
| 劫财 | 同我者，阴阳异 | 姐妹、破财、冲动 |
| 食神 | 我生者，阴阳同 | 子女、才华、享受 |
| 伤官 | 我生者，阴阳异 | 子女、创意、叛逆 |
| 偏财 | 我克者，阴阳同 | 父亲、偏财、投机 |
| 正财 | 我克者，阴阳异 | 妻子、正财、稳定 |
| 七杀 | 克我者，阴阳同 | 情人、压力、权威 |
| 正官 | 克我者，阴阳异 | 丈夫、事业、名誉 |
| 偏印 | 生我者，阴阳同 | 继母、特殊才能 |
| 正印 | 生我者，阴阳异 | 母亲、学历、贵人 |

### 大运排法

**规则：**
- 阳男阴女顺排，阴男阳女逆排
- 每运 10 年
- 简化：3 岁起运

---

## 6. 日主性格断语

| 日主 | 断语 |
|------|------|
| 甲木 | 甲木参天，正直仁慈，有上进心 |
| 乙木 | 乙木柔顺，温和善良，适应力强 |
| 丙火 | 丙火太阳，热情开朗，表现欲强 |
| 丁火 | 丁火灯烛，温和内敛，心思细腻 |
| 戊土 | 戊土大地，厚重诚实，包容力强 |
| 己土 | 己土田园，细腻谨慎，善于谋划 |
| 庚金 | 庚金刀剑，刚毅果断，执行力强 |
| 辛金 | 辛金珠玉，温润秀气，重面子 |
| 壬水 | 壬水江河，聪明灵活，适应力强 |
| 癸水 | 癸水雨露，温柔内敛，直觉敏锐 |

---

## 7. 示例

### 示例 1：基础排盘

**用户输入：**
> "帮我排一下 1990 年 5 月 15 日早上 8 点的八字"

**命令：**
```bash
python3 scripts/bazi_pan.py --date "1990-05-15" --hour 8
```

**输出：**
```
【八字排盘】v2.0.0
• 出生时间：1990-05-15 08:00
• 性别：男

【四柱】
    年柱    月柱    日柱    时柱
    庚午    辛巳    壬辰    甲辰

【十神】
    偏印    正印    日主    食神

【五行统计】
    木：2  火：2  土：2  金：2  水：0
    ⚠️  五行缺：水

【大运】
    13 岁：壬午
    23 岁：癸未
    ...

【格局】
    正印格

【用神】
    日主水弱，喜金,水，忌土,火

【详细断语】
• 【日主】壬水江河，聪明灵活，适应力强
• 【格局】正印格：聪明智慧，利学业文化
• 【五行】五行缺水，需后天补救
```

### 示例 2：女性排盘

**用户输入：**
> "1995 年 8 月 20 日下午 3 点，女"

**命令：**
```bash
python3 scripts/bazi_pan.py --date "1995-08-20" --hour 15 --gender 女
```

### 示例 3：流年推算

**用户输入：**
> "看看 2026 年的流年运势"

**命令：**
```bash
python3 scripts/bazi_pan.py --date "1990-05-15" --hour 8 --liu-nian 2026
```

**输出新增：**
```
【丙午流年】
    流年十神：七杀
    冲克：无重大冲克
    断语：事业有冲劲，宜创业或开拓性工作
```

### 示例 4：JSON 输出

**用户输入：**
> "要 JSON 格式的八字"

**命令：**
```bash
python3 scripts/bazi_pan.py --date "1990-05-15" --hour 8 --json
```

---

## 8. GitHub 仓库

**仓库地址：** https://github.com/dglijin-oss/bazi-pan-skill

**版本历史：**
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v4.0.0 | 2026-04-25 | 新增刑冲合害分析、流月推算、综合评分、趋吉避凶建议四大模块 |
| v3.3.0 | 2026-04-14 | 新增神煞增强模块（33+神煞，识别率100%） |
| v3.1.0 | 2026-04-11 | 统一断语风格、技能联动接口、十大技能协同 |
| v2.0.0 | 2026-03-24 | 格局判断、用神选取、流年推算、增强断语库 |
| v1.0.0 | 2026-03-22 | 基础四柱排盘、十神、大运、五行统计 |

**安装方式：**
```bash
git clone https://github.com/dglijin-oss/bazi-pan-skill.git
cd bazi-pan-skill
python3 scripts/bazi_pan.py --help
```

---

## 9. 后续迭代计划

### v2.0.0（✅ 已完成 2026-03-24）
- ✅ 流年推算（当年运势、冲克分析）
- ✅ 格局判断（正官格/七杀格/食神格等 10 种格局）
- ✅ 用神选取（日主旺衰、喜用神、忌神）
- ✅ 详细断语（事业/婚姻/财运/健康）
- ✅ 增强断语库（分类断语：事业/婚姻/财运/健康）

### v3.0.0（✅ 已完成 2026-04-14）
- ✅ 神煞系统（33+神煞）
- ✅ 刑冲合害分析（地支六冲/六合/三合等）

### v4.0.0（✅ 已完成 2026-04-25）
- ✅ 刑冲合害分析完整模块（六合/六冲/三合/三刑/六害/相破）
- ✅ 流月推算（12个月运势，事业/财运/感情/健康）
- ✅ 综合评分（六维度，0-100分）
- ✅ 趋吉避凶建议（方位/颜色/行业/数字/吉祥物/食物）

### v5.0.0（预计 2026-09-30）
- [ ] 流月+流年+大运联动分析
- [ ] 精确节气计算（替代简化版月柱计算）
- [ ] 真太阳时校正
- [ ] 八字合婚配对模块
- [ ] 命盘可视化（图表输出）
- [ ] 历史卦例数据库（500+案例）

---

> **天工长老按：** 八字命理，乃人生之地图。知命者不忧，知理者不惑。v4.0 已具备完整排盘分析能力，后续将向精确化和可视化方向精进。

🏮 天工长老 敬撰 v1.0.0
2026-03-22
